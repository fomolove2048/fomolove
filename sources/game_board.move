module fomolove2048::game_board {
    ...
    struct GameBoard has store, copy, drop{
        packed_spaces: u64,
        score: u64,
    ...
    public(friend) fun default(random: vector<u8>): GameBoard {
    ...
    public(friend) fun move_direction(game_board: &mut GameBoard, direction: u64, random: vector<u8>) {
    ...
    public(friend) fun packed_spaces(game_board: &GameBoard): &u64 { 
        &game_board.packed_spaces 
    }

    public(friend) fun score(game_board: &GameBoard): &u64 { 
        &game_board.score 
    }

    public(friend) fun last_tile(game_board: &GameBoard): &vector<u64> {
        &game_board.last_tile
    }

    public(friend) fun top_tile(game_board: &GameBoard): &u64 {
        &game_board.top_tile
    }

    public(friend) fun game_over(game_board: &GameBoard): &bool {
    ...
    public(friend) fun board_space_at(game_board: &GameBoard, row_index: u8, column_index: u8): u64 {
        space_at(game_board.packed_spaces, row_index, column_index)
    }

    public(friend) fun empty_space_positions(game_board: &GameBoard, direction: u64): vector<SpacePosition> {
    ...
    public(friend) fun empty_space_count(game_board: &GameBoard): u64 {
        vector::length(&empty_space_positions(game_board, ALL))
    }

    public(friend) fun analyze_top_tile(game_board: &GameBoard): u64 {
        let top_tile = 0;
    ...
    fun move_possible(game_board: &GameBoard): bool {
        let rows = ROW_COUNT;
        let columns = COLUMN_COUNT;
    ...
    fun add_new_tile(game_board: &mut GameBoard, direction: u64, random: vector<u8>): u64 {
    ...
    #[test_only]
    public fun print_board(board: &GameBoard) {        
    ...
    #[test_only]
    fun game_board_matches(game_board: &GameBoard, expected_spaces: vector<u64>): bool {
