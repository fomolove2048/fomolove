...
#[test_only]
module fomolove2048::leaderboard_tests {
    use std::option;
    use std::vector;
    ...
    use fomolove2048::leaderboard::{Self, Leaderboard8192};
    ...
    fun achieve_score(scenario: &mut Scenario, score: u64) {
        ...
        test_scenario::next_tx(scenario, PLAYER);
        {
            ...
            leaderboard::submit_game(&mut game, &mut leaderboard);
    ...
    fun achieve_score_game(scenario: &mut Scenario, game_id: ID, score: u64) {
        test_scenario::next_tx(scenario, PLAYER);
        {
            ...
            let ctx = test_scenario::ctx(scenario);
            while (*game::score(&game) < score) {
                game::make_move(&mut game, left(), ctx);
                game::make_move(&mut game, up(), ctx);
                game::make_move(&mut game, right(), ctx);
                game::make_move(&mut game, down(), ctx);
            };
    ...
    fun check_scores_for_leaderboard(leaderboard: &Leaderboard8192, scores: vector<u64>) {
        ...
        while (index < vector::length(&scores)) {
            let top_game = leaderboard::top_game_at(leaderboard, index);
            assert!(leaderboard::top_game_score(top_game) == vector::borrow(&scores, index), index);
            index = index + 1;
        };
        let top_games_length = vector::length(leaderboard::top_games(leaderboard));
    ...
    fun test_submit_game() {
        ...
        leaderboard::create(test_scenario::ctx(&mut scenario));
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard::top_game_at(&leaderboard, 0);
            assert!(leaderboard::top_game_game_id(top_game) == game::id(&game), 1);
    ...
    fun test_submit_game__inserts_game_at_correct_location_in_top_list() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard::top_game_at(&leaderboard, 0);
            assert!(leaderboard::top_game_game_id(top_game) == game::id(&game), 0);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard::top_game_at(&leaderboard, 1);
            assert!(leaderboard::top_game_game_id(top_game) == game::id(&game), 1);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard::top_game_at(&leaderboard, 3);
            assert!(leaderboard::top_game_game_id(top_game) == game::id(&game), 1);
    ...
    fun test_submit_game__only_one_entry_per_game() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let leaderboard_game_count = vector::length(leaderboard::top_games(&leaderboard));
    ...
    #[test]
    #[expected_failure(abort_code = leaderboard::ELowScore)]
    fun test_submit_game__aborts_if_not_a_leader() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 2, 0, 0);
    ...
    fun test_submit_game__stores_min_tile_and_min_score() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            assert!(leaderboard::min_tile(&leaderboard) == &0, (*leaderboard::min_tile(&leaderboard) as u64));
            assert!(leaderboard::min_score(&leaderboard) == &0, *leaderboard::min_score(&leaderboard));
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            assert!(leaderboard::min_tile(&leaderboard) == &2, (*leaderboard::min_tile(&leaderboard) as u64));
            assert!(leaderboard::min_score(&leaderboard) == &4, *leaderboard::min_score(&leaderboard));
            
            assert!(game::score(&game) == &16, *game::score(&game));
    ...
    #[test]
    #[expected_failure(abort_code = leaderboard::ELowTile)]
    fun test_submit_game__aborts_if_not_above_min_tile() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 2, 3, 30);
    ...
    fun test_submit_game__aborts_if_not_above_min_score() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 2, 0, 30);
    ...
    fun test_submit_game__accepts_game_if_above_min_score_even_if_leaderboard_full_removes_last_entry() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_games = leaderboard::top_games(&leaderboard);
            let leaderboard_game_count = vector::length(top_games);
            assert!(leaderboard_game_count == 2, leaderboard_game_count);
            
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game3_id == leaderboard::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game2_id == leaderboard::top_game_game_id(top_game1), 1);
    ...
    fun test_submit_game__top_game_removes_bottom() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 3, 0, 0);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game::id(&game4) == leaderboard::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game::id(&game3) == leaderboard::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(game::id(&game2) == leaderboard::top_game_game_id(top_game2), 2);
    ...
    fun test_submit_game__middle_game_removes_bottom() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game::id(&game3) == leaderboard::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game::id(&game1) == leaderboard::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(game::id(&game4) == leaderboard::top_game_game_id(top_game2), 2);
    ...
    fun test_submit_game__bottom_game_removes_bottom() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game::id(&game1) == leaderboard::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game::id(&game4) == leaderboard::top_game_game_id(top_game1), 1);
    ...
    fun test_lots_of_games() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 9, 0, 0);
    ...
    fun test_move_one_game_around() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard::blank_leaderboard(&mut scenario, 5, 0, 0);
    ...
    fun test_lots_of_players() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(option::destroy_some(game_2_id) == leaderboard::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(option::destroy_some(game_3_id) == leaderboard::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(option::destroy_some(game_1_id) == leaderboard::top_game_game_id(top_game2), 2);
    ...
    fun assert_sort(top_games: vector<leaderboard::TopGame8192>, expected: vector<vector<u64>>) {
        let sorted = leaderboard::merge_sort_top_games(top_games);
        assert_sorted(sorted, expected);
    }

    fun assert_merge(left: vector<leaderboard::TopGame8192>, right: vector<leaderboard::TopGame8192>, expected: vector<vector<u64>>) {
        let merged = leaderboard::merge(left, right);
        assert_sorted(merged, expected);
    }

    fun assert_sorted(sorted: vector<leaderboard::TopGame8192>, expected: vector<vector<u64>>) {
        ...
        while (!vector::is_empty(&sorted)) {
            ...
            assert!(score == *leaderboard::top_game_score(&top_game), score);
            assert!(tile == *leaderboard::top_game_top_tile(&top_game), tile);
        }
    }
    ...
    fun test_sort() {
        ...
        let top_games = vector<leaderboard::TopGame8192>[
            leaderboard::top_game(&mut scenario, PLAYER, 2, 200),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 500),
        ];
        ...
        let top_games = vector<leaderboard::TopGame8192>[
            leaderboard::top_game(&mut scenario, PLAYER, 2, 200),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 500),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 400),
        ];
        assert_sort(top_games, vector[
            vector[2, 500], 
            vector[2, 400], 
        ...
        let top_games = vector<leaderboard::TopGame8192>[
            ...
            leaderboard::top_game(&mut scenario, PLAYER, 2, 500),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 400),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 300),
        ];
        assert_sort(top_games, vector[
            vector[2, 500], 
            vector[2, 400], 
        ...
        let top_games = vector<leaderboard::TopGame8192>[
            leaderboard::top_game(&mut scenario, PLAYER, 1, 200),
            leaderboard::top_game(&mut scenario, PLAYER, 4, 300),
            leaderboard::top_game(&mut scenario, PLAYER, 2, 400),
            leaderboard::top_game(&mut scenario, PLAYER, 3, 500),
        ];
        assert_sort(top_games, vector[
            vector[4, 300], 
            vector[3, 500], 
    ...
    fun test_merge() {
        ...
        let left = vector<leaderboard::TopGame8192>[
            leaderboard::top_game(&mut scenario, PLAYER, 2, 200),
        ];
        let right = vector<leaderboard::TopGame8192>[
            leaderboard::top_game(&mut scenario, PLAYER, 2, 500),
        ];
